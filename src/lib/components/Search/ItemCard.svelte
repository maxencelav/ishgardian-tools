...
<li class="w-full rounded overflow-hidden drop-shadow-md bg-orange-100 dark:bg-zinc-800 dark:text-zinc-200 flex flex-col border border-stone-500/30 z-20">
    ...
    <p class="bg-stone-700 text-stone-300 dark:bg-zinc-950/50 dark:text-zinc-300 w-full text-xs uppercase tabular-nums inline-block px-2 pb-0.5 font-wide mb-2">
    ...
    <div class="px-2 text-xs mb-1">
        {#each Object.entries(languages) as [code, name]}
            <p class="text-zinc-700 dark:text-zinc-300 mb-1 select-none flex items-start">
                <span class="bg-zinc-500/50 text-zinc-800 inline-block text-center w-5 mr-2 py-0.5 rounded dark:bg-zinc-700 dark:text-zinc-300 select-none">
                    {code[0].toUpperCase()}
                </span>
                <ClickToCopy 
    ...
    <p class="px-2 mb-2 text-stone-300 dark:text-zinc-300 text-sm">
